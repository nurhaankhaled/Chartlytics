...
class HistoryState extends State<history> {
  ...
  print(PickImageState.email);
    ...
    for(int i = 0; i < map.keys.toList().length; i++){
      if(map.keys.toList()[i]!= "last name" || map.keys.toList()[i]!= "first name")
        {
          imageID = map.keys.toList()[i];
          // break;
        }
    };
    // imageID = map.keys.toList()[0];
    summary = map[imageID];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Center(
            child: Text('Chartlytics', style: TextStyle(color: Colors.black54,
                fontSize: 50,
    ...
    child: SingleChildScrollView(
        child: Column(
        children: [
        Image.network(url,height:250 ,fit: BoxFit.cover,loadingBuilder: (context, child, loadingProgress)
    {
